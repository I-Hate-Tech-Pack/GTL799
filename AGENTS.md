...
### 1. `artifact Jar` 报错 `Cannot convert to MavenArtifact`

**错误信息**：
```
Cannot convert the provided notation to an object of type MavenArtifact: class org.gradle.api.tasks.bundling.Jar.
```

**原因**：`artifact Jar` 中的 `Jar` 是 Gradle 任务**类**（`org.gradle.api.tasks.bundling.Jar`），而 `artifact` 需要的是任务**实例**（即 `jar` 任务）。

**判定方法**：CI 日志中出现上述错误信息即可判定。

**修复**：将 `artifact Jar` 改为 `artifact jar`（小写）。

> **原则**：在 Gradle 中，`Jar`（大写）是类名，`jar`（小写）是默认创建的 Jar 任务实例名。`artifact` 只能接收任务实例、文件路径或 `PublishArtifact`，不能接收类引用。
...
- **`artifact Jar` vs `artifact jar`**：`Jar`（大写）是类，`jar`（小写）是任务实例。`artifact` 必须使用小写 `jar`，否则 CI 会报 `Cannot convert to MavenArtifact`
